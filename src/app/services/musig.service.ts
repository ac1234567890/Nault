...
export class MusigService {
  ...
  aggregate(storedAccounts, runWithPubkeys = null) {
    ...
    for (let address of addresses) {
      ...
      if (!address.startsWith('xrb_') && !address.startsWith('co2c_')) {
        throw new Error('Nano addresses must start with xrb_ or co2c_');
      }
    ...
    const fullAddressFinal = 'co2c_' + base32.encode(fullAddress);
