...
export class MultisigComponent implements OnInit {
  ...
  addAccount() {
    ...
    if (this.storedAccounts.includes(this.accountAdd.replace('xrb_', 'co2c_').toLocaleLowerCase())) {
    ...
    this.storedAccounts.push(this.accountAdd.replace('xrb_', 'co2c_').toLocaleLowerCase());
    this.accountAdd = '';
  ...
  navigateAccount() {
    if (this.validateMultisig()) {
      ...
      this.notificationService.sendWarning('Invalid co2c account!');
    }
  }
