...
export class AccountDetailsComponent implements OnInit, OnDestroy {
  ...
  remoteVisible = false;
  blockTypes: string[] = ['Send Co2c', 'Change Representative'];
  ...
  async generateSend() {
    ...
    if (!this.validateAmount()) return this.notifications.sendWarning(`Invalid CO2C Amount`);
    ...
    if (from.balanceBN.minus(rawAmount).lessThan(0)) return this.notifications.sendError(`From account does not have enough CO2C`);
    ...
    const blockData = {
      account: this.accountID.replace('xrb_', 'co2c_').toLowerCase(),
    ...
    const blockDataPrevious = {
      account: jsonBlock.account.replace('xrb_', 'co2c_').toLowerCase(),
